...
First i research how to make a linked list that would be suited for this problem decided to go with concurrent linkedlist.
After I decided to create a bag(vector) thats sorted as initially it would be. After I swap all items around the bag to make it unordered to simulate the problem. I have 4 servants so 4 threads and in each of these threads they either insert or remove and in the remove we also write the thank you cards.Keep in mind i have it so that as it inserts elements into the vector it sorts them too to make sure it fits problem same thing with the remove.The reason why I chose a concurrent linked list to a parralel is because for some reason with the parralel it would sometimes add same element twice or try to remove twice as threads can take same variable ifg not locked.
# How to run
compile
g++ assignment3problem1.cpp
then to run
.\a.exe  
...
First I realized I would need a random number generator but instead of previous one it would need to be uniform distribution so I did that. Then made functions to check if my threads/sensors are ready then to perform the tasks stated in problem 2
# How to run
compile
g++ assignment3problem2.cpp
then to run
.\a.exe 
